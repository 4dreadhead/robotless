...
async def get_all(request: Request):
    ...
    if not (pf_name in PLATFORMS and name in BROWSERS):
        return share_response({"User-Agent": ua_row}, Code.CANT_RECOGNIZE)

    client_hello = request.headers.get("X-Client-Hello")

    if not client_hello:
        return share_response({"User-Agent": ua_row}, Code.CANT_COLLECT)
    ...
    if fp not in parsed_tool.fingerprints:
        ...
        return share_response({"User-Agent": ua_row}, Code.ALREADY_EXISTS)


def share_response(response, code):
